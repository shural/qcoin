...
contract Event {
    ...
    /// @dev Buys tokens for specific outcomes, exchanging collateral tokens and sets of outcome tokens 1:1
    /// @param collateralTokenCount Number of collateral tokens
    function buySpecificOutcome(uint collateralTokenCount, uint outcomeIndex, address sender)
        public
    {
        // Transfer collateral tokens to events contract
        require(collateralToken.transferFrom(sender, this, collateralTokenCount));
        // Issue new outcome tokens to sender
        outcomeTokens[outcomeIndex].issue(sender, collateralTokenCount);
        OutcomeTokenSetIssuance(sender, collateralTokenCount); 
    }
